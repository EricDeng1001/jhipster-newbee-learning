...
import me.antinux.domain.Entry;
import me.antinux.repository.EntryRepository;
...
/**
 * REST controller for managing {@link me.antinux.domain.Entry}.
 */
@RestController
@RequestMapping("/api")
public class EntryResource {
    ...
    private boolean isNotAccessingSelf(Entry entry) {
        return false;
    }
    ...
    public ResponseEntity<Entry> createEntry(@Valid @RequestBody Entry entry)
        ...
        if (isNotAccessingSelf(entry) && !SecurityUtils.isCurrentUserInRole(ADMIN)) {
     ...
     */
    @PutMapping("/entries")
    public ResponseEntity<Entry> updateEntry(@Valid @RequestBody Entry entry)
        throws URISyntaxException, AccessDeniedException {
    ...
    public ResponseEntity<List<Entry>> getAllEntries(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        Page<Entry> page = entryRepository.findAllByBlogUserLoginOrderByDateDesc(SecurityUtils.getCurrentUserLogin().orElse(null), pageable);
